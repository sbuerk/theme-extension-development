...
- The development instances, one per supported core version —
  ...
  `runTests.sh`. They are installed with `ddev composer` or from the host, and
  the two must not be mixed — the resulting `vendor/` differs and does not
  travel. `ddev start` sets an instance up and seeds it
...
- The instances are built from the seed set `theme-instance` of
  ...
  extension and mirrors it below `/legacy/` — the v12 instance from
  `theme-instance-core12`, which adds the root `sys_template` record the `/`
  tree needs without site sets. `ScenarioLegacy.yaml` and
