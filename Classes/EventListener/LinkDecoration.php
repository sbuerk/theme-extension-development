 ...
 *
 * Registered by the "event.listener" tag rather than "#[AsEventListener]",
 * which TYPO3 v12.4 does not have. The "ListenerProviderPass" of both cores
 * reads the "identifier" of the tag and, without an "event", takes the event
 * from the type of the first parameter of "__invoke()". The "config." of the
 * page is read through "FrontendConfigInterface", which differs per core.
 *
 * @todo Replace the tag with "#[AsEventListener]" when TYPO3 v12 is dropped.
 */
...
final class LinkDecoration
{
    ...
    public function __construct(
        ...
        private readonly FrontendConfigInterface $frontendConfig,
    ) {}
    ...
    {
        // Without "config." the request is not a page rendering.
        $config = $this->frontendConfig->forRequest($request) ?? [];
