 ...
 *
 * Invoked through the composer scripts of "instance-core-12/" and
 * "instance-core-13/", so the working directory is the instance directory and
 * every path is resolved relative to it. There are no path arguments on
 * purpose. The one argument besides the mode is the seed set, because the two
 * instances are built from different ones: TYPO3 v12 has no site sets, and its
 * instance imports "theme-instance-core12", which adds the root "sys_template"
 * record its "/" tree is delivered through - see "docs/development/seeding.md".
 *
 *   composer system:setup    php ../theme/Build/Scripts/instance.php setup <seed-set>
 *   composer system:reseed   php ../theme/Build/Scripts/instance.php reseed <seed-set>
 *
...
function main(array $argv): int
{
    $mode = $argv[1] ?? '';
    $seedSet = $argv[2] ?? '';
    if (!in_array($mode, ['setup', 'reseed'], true) || preg_match('/^[a-z0-9][a-z0-9-]*$/', $seedSet) !== 1) {
        fwrite(STDERR, "Usage: instance.php <setup|reseed> <seed-set>\n");
        return 1;
    }
    ...
    if (!is_file(ADDITIONAL_FILE) || !is_file('composer.json')) {
        fwrite(STDERR, sprintf(
            ...
            . "Run this through \"composer system:%s\" in \"instance-core-12/\" or \"instance-core-13/\".\n",
            getcwd() ?: '.',
            $mode,
        ));
        return 1;
    ...
    return setup($seedSet);
}

function setup(string $seedSet): int
{
    ...
    section('Seeding "' . $seedSet . '"');
    foreach ([
        ['extension:setup'],
        ['data-factory:import', $seedSet, '--no-interaction'],
        ['dev-site:seed-state', '--mark=' . $seedSet],
        ['cache:flush'],
    ] as $command) {
