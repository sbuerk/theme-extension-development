...
# ones at the repository root. Non-existing directories are skipped.
scanPaths=("Classes/" "Core13/" "Core14/")
...
scan_exceptions() {
    ...
    local existingScanPaths=()
    local candidate=""
    for candidate in "${scanPaths[@]}"; do
        if [ -d "$candidate" ]; then
            existingScanPaths+=("$candidate")
        fi
    done

    if [ ${#existingScanPaths[@]} -eq 0 ]; then
        echo "None of the configured scan paths exists: ${scanPaths[*]}"
        exit 1
    fi
