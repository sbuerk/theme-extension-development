...
A new column :sql:`tx_theme_list_item.brand_icon`, declared in
:file:`ext_tables.sql` like every other column of that table - TYPO3 v12.4
derives no column from a select - as the :sql:`VARCHAR(255)` TYPO3 v13.4
would derive from the TCA, so the database analyser adds it on both
versions and asks for no change afterwards. Existing records are
unaffected: the column is empty, and an entry without a logo renders as the
text link it rendered before.
