...
``fluid_styled_content``. An installation that has both can now say so, with a
site set of its own:
...
For an installation without site sets there is a matching static template,
...
override at ``10`` — identical in that extension's 13.4 and 14.3 releases, read
