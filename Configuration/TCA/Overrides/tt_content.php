...
$additionalColumns = [
    ...
    // installations already store the style in it, with the three values
    // before "link", so renaming it would lose it.
...
//
// "selectSingle" of string values. v12.4 derives no column from a "select"
// without an MM table, so each is declared in "ext_tables.sql" as the
// "VARCHAR(255) DEFAULT '' NOT NULL" v13.4 derives - with an empty default,
// not the TCA one. The template maps every value it does not know - an empty
// one included - to the default of its axis.
...
// 12.4.45 and 13.4.35). So the theme adds a field of its own.
//
...
// "bullets" is registered by EXT:frontend - in its base TCA on v12.4, in a
// TCA override on v13.4, and EXT:frontend's overrides run before this
// extension's - so its type exists here on both.
