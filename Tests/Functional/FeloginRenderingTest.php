...
final class FeloginRenderingTest extends AbstractFunctionalTestCase
{
    ...
    /**
     * TYPO3 v13 assigns the logout redirect target as "actionUri", and the
     * form posts there: logging out on "/redirects" lands on "/goodbye".
     */
    #[Group('not-core-14')]
    #[DataProvider('deliveries')]
    #[Test]
    public function onTypo3v13TheLogoutFormPostsToTheLogoutRedirectTarget(string $delivery): void
    {
    ...
    /**
     * TYPO3 v14 removed "actionUri" and redirects through an event after the
     * logout (Breaking #103910): the form posts to the page it is on, and the
     * unset argument must not send it anywhere else.
     */
    #[Group('not-core-13')]
    #[DataProvider('deliveries')]
    #[Test]
    public function onTypo3v14TheLogoutFormPostsToThePageItIsOn(string $delivery): void
    {
        $form = $this->tag($this->renderThrough($delivery, '/redirects', true), 'form', 'action="');

        $this->assertStringContainsString('/redirects', $form);
        $this->assertStringNotContainsString('/goodbye', $form);
    }
}
