 ...
 * the theme delivery of the running core (`ThemeSiteTrait`, the site set on
 * v13, the static include on v12) and the static include as such.
 *
...
final class ThemeContentElementObjectTest extends AbstractFunctionalTestCase
{
     ...
     *
     * A theme delivery through a set gets a `sys_template` record of its own
     * for the clearing line, without a `clear` flag: a record with one starts
     * the TypoScript from nothing, the set included
     * (`IncludeTreeAstBuilderVisitor`), and `setUpFrontendRootPage()` always
     * writes one. A theme delivery that is a `sys_template` record itself -
     * v12's - carries the clearing line after its static include, in the same
     * record, like the static path. The static path includes the static
     * template the way an installation does, as `include_static_file`,
     * followed by the clearing line in the same record.
     */
    private function setUpSite(string $path, bool $clearContentElement): void
    {
        ...
        if ($path === 'theme' && $delivery->createsSysTemplateRecord()) {
            $this->setUpFrontendRootPage(1, [], $delivery->templateValues() + ['config' => $setup]);
        } elseif ($path === 'theme') {
            $this->setUpFrontendRootPage(1, [], $delivery->templateValues(), false);
