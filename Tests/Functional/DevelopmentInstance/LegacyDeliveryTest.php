 ...
 * The `/` tree is delivered through the site set of the theme, the `/legacy/`
 * tree through the `include_static_file` of a root `sys_template` record - and
 * the second mechanism fails silently. The column is a comma separated list
...
final class LegacyDeliveryTest extends AbstractInstanceSeedTestCase
{
    ...
    public function bothTreesDeliverTheSameMarkupForEveryMirroredPage(): void
    {
        ...
        $this->assertSame(
            ...
            sprintf(
                ...
                . ' saying the same thing. A difference here is usually an entry of "include_static_file" that'
                . ' resolves to nothing, which raises nothing at runtime.',
                count($differences),
                count($pages),
                implode("\n  ", $differences),
            ),
    ...
    /**
     * There is exactly one TypoScript template record, and it is the root of
     * the legacy tree.
     *
