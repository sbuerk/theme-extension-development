...
/**
 * The setting `theme.header.variant` picks one of four header partials.
 *
 ...
 * setting written where an integrator of the running core writes it: into
 * the site configuration as a site setting of the set on TYPO3 v13, and as
 * the TypoScript constant of the same name in the `sys_template` record of
 * the static include on TYPO3 v12, which has no site sets - and where a
 * site setting of the same key would be added as a constant before the
 * static include and lose to the theme's default. On v12 this class is
 * therefore the proof that the constant alone delivers every variant; see
 * {@see ThemeDeliveryInterface}.
 *
...
final class SiteHeaderVariantRenderingTest extends AbstractFunctionalTestCase
{
     ...
     *
     * Where the settings go is decided by the theme delivery of the running
     * core: on v13 into the site configuration as a **map** of the full
     * setting keys, which is the format the core stores and advises since
     * TYPO3 v13.4 (#106894, "Site settings.yaml is now stored as a map"); on
     * v12 into the constants of the `sys_template` record that selects the
     * static include.
     *
    ...
    {
        $this->setUpThemeSite(settings: $settings);
