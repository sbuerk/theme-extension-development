...
A run takes about 60 seconds of specs for 225 tests. It is **not** a
...
|------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|-------|
| axe, WCAG 2.2 AA       | every section × light, dark × every palette                                                                                                         | 150   |
| Screenshot, clipped    | every section × light, dark in `neutral`; the accent swatches and the focus ring of `tokens` and the whole of `buttons` × light, dark × four others | 54    |
| Info follows primary   | the `info` swatch of `tokens` shows the primary accent, every appearance and palette                                                                | 10    |
| Switch thumb           | the switch of `forms` that is off paints its thumb at the start of the track, the one that is on at the end, every appearance and palette           | 10    |
...
`tokens` as `tokens-accents-…` and `tokens-focus-…` — 20.4 MB for the 54 today.
...
ViewHelper, a variable or a setting only TYPO3 provides fails there. The v12 set
(Fluid 2) and the v13 set (Fluid 4) render identical sections.

## In CI

The job `visual` runs once, after `quality`, on the v12 set and the lowest PHP
