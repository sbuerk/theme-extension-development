...
| The request type branch in `UnitTestsBootstrap.php`                       | The 8.x boilerplate calls `SystemEnvironmentBuilder::run()` with `REQUESTTYPE_CLI` unconditionally. This branch on `class_exists(\TYPO3\CMS\Core\Http\Application::class)` adds `REQUESTTYPE_BE` where that class is absent, which is the case on TYPO3 v12.4, and **must be kept** — the bootstrap says so as well. |
