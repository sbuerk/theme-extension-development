...
`file.type`, read through `getType()`, which returns the same `int` on 12.4.45
and 13.4.35 (verified in `AbstractFile::getType()`). The integers are the
`FILETYPE_*` constants of `AbstractFile` on v12.4 and the cases of the
`FileType` enum that superseded them on v13.4 (#102032, 13.0, which deprecated
the constants and not the method) — the same values either way, so no version
split is needed:
...
any notion of a text track — there is no `track` in either class, on 12.4.45
or 13.4.35. A caption track is what WCAG 1.2.2 asks for, and it cannot come
from a renderer that cannot produce one.
...
theme's delivery on the running core - the set on v13, the static include on
v12 - and through the static include, against a real indexed file as page
media: the image and its alternative text, the title as the link, the abstract
on cards only, a page without media, the list for layout 0 and for the removed
...
On TYPO3 v13 the whole schema for `tx_theme_list_item` and the twenty-six
...
without an MM table — so on v12 the table and the twenty-six columns are simply
...
column from `dbType` on both cores - v12.4 too, whose `DefaultTcaSchema` has a
branch for `datetime`, so `ext_tables.sql` does not declare it - and
DataHandler stores an empty value as NULL (`checkValueForDatetime()`, read on
v12.4.45 and v13.4.35). It is written out with the ICU
...
(`PageLinkBuilder::parseFallbackLinkTextIfLinkTextIsEmpty()`, read on v12.4.45
and v13.4.35), the address for an email link, the URL otherwise - rather than the
...
the database default 0 described above, which is why the template
...
`CollectionElementRenderingTest` renders all three through the theme's
delivery on the running core - the set on v13, the static include on v12 -
and through the static include: the column modifiers and the scroller
region, the defaults for values the form does not offer, the date order with a
tie in both directions, the date as `time`, the icon of an entry, and one link
per row.
...
That block is not written for every plugin. On both supported versions it is
the `PLUGIN_TYPE_CONTENT_ELEMENT` case of a `switch` on the plugin type (lines
72–74 of that file at `v12.4.45`, 77–79 at `v13.4.35`), and the type still
defaults to `PLUGIN_TYPE_PLUGIN` (`list_type` — the parameter default at line
48 on `v12.4.45`, assigned at line 55 on `v13.4.35`), whose case writes
`tt_content.list.20.<pluginSignature>` instead — see `tt_content.list` below.
Of the TYPO3 system extensions, only `fluid_styled_content` defines the
object, on v12.4 and v13.4 alike, in its
