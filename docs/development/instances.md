...
name, their website title, the seed set they are built from — and in **how the
theme is enabled**, which is the one difference that is not cosmetic; see
...
Three composer scripts, the same in both instances but for the seed set they
pass on — `theme-instance` in `instance-core-13/`, `theme-instance-core12` in
`instance-core-12/`:
...
2. **Is it seeded?** Yes when `vendor/bin/typo3 dev-site:seed-state` exits `0`.
   If not, `extension:setup`, `data-factory:import <set>`,
   `dev-site:seed-state --mark=<set>` and `cache:flush` run, in that order.
...
| Path                               | Is                                                                       |
|------------------------------------|--------------------------------------------------------------------------|
| `.ddev/config.yaml`                | The DDEV project, `core12-theme-v1` / `core13-theme-v1`, and its hooks.  |
...
so on a site set site the plugin otherwise runs with no configuration at all —
and TYPO3 v13's login controller then fails on the logout form of a logged in
user. On TYPO3 v12 both trees are `sys_template` sites, which that
registration reaches. All three pages carry `nav_hide`, because the `/legacy/`
tree mirrors the showcase and not these pages, and a login link in one tree's
navigation only would make every page of the two trees differ.
...
The path is `var/sqlite/core-13.sqlite` (`core-12.sqlite`), and
...
branch, `core13-theme-v2` and `core14-theme-v2` on `main`. The second half is
what matters here: `instance-core-13/` exists on both branches, the instance
