...
preset. On v13.4 it is part of EXT:core, and `Richtext::getConfiguration()`
calls it as its last step. On v14.3 it moved to rte_ckeditor and runs from an
`AfterRichtextConfigurationPreparedEvent` listener
(`Classes/EventListener/AfterRichtextConfigurationPreparedEventListener.php`).
On both, while the alignment plugin is loaded, `handleAlignmentPlugin()` writes
all four alignments. An alignment the preset does not name gets the core's
class — `text-justify` for this one (read on v13.4.35 and v14.3). The
...
`Processing.yaml` is the same file on v13.4 and v14.3 but for one line: v14
adds `font` and `center` to `removeTags` (Important #106192). The preset
inherits whichever the installed core ships, and nothing in the theme depends
on the difference.
...
The registration is the same on v13.4 and v14.3. `Richtext` reads
...
site takes its own preset back in any of the later three:
