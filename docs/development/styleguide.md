...
`Templates/Page/Styleguide.html`, not in any of the eleven partials — not even
...
## Eleven sections, eleven partials
...
is what places them, and the index at the top is built from the same eleven ids.
...
`Partials/Styleguide/Forms.html`, and keeps the other ten. Overriding the page
template instead would mean re-stating all eleven renders and the index to
change one section.
...
| `everySectionOfTheStyleguideRendersAndIsLinkedFromTheIndex` | Each of the eleven ids renders **and** is linked from the index — no dead anchor.                                         |
